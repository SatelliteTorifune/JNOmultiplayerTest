...
namespace Assets.Scripts.Net.MultiPlayerTransport
{
	...
	/// 让 MultiPlayerNetworkManager 的 Transport 字段可在两者间切换（TCP 用于本地虚拟机 debug），
	...
	{
		event Action<MultiPlayerPeer, byte[]> OnDataReceived;
		event Action<MultiPlayerPeer> OnPeerTimeout;
		...
		void Stop();
		void DrainIncoming();
		void SendTo(MultiPlayerPeer peer, byte[] data);
		void Broadcast(byte[] data);
		void CheckTimeouts(long timeoutMs);
		IReadOnlyCollection<MultiPlayerPeer> GetPeers();
		int GetPeersCount();

		/// <summary>房主：断开与指定对端的连接（踢人用）。断开后传输层会移除该 peer 并触发 OnPeerTimeout。</summary>
		void DisconnectPeer(MultiPlayerPeer peer);
	}
}
