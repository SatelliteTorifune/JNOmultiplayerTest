...
## 〇、实现状态核对(2026-09 代码复核)

> 本文件的方案**一项都还没实现**。当前代码在"船"这个维度上仍是**每玩家一艘船**,且没有任何为多 craft 预留的数据结构。落地时不要假设下面这些"已经有一半了"。

| 能力 | 计划要求 | 代码现状 | 证据 |
|---|---|---|---|
| **跨机 Craft 身份(Guid)** | mod 自生成 `Guid` + `nodeId→Guid` 映射,状态包带 `(ownerId, craftGuid)` | ❌ **完全没有** | 全仓库 `Guid`/`CraftGuid` **0 命中**;对端表是 `Dictionary<int, RemoteCraft> _remoteCrafts`,**key = int playerId**(`MpNetworkManager.cs:59`) |
| **每玩家多船** | 一名玩家可上报多艘船 | ❌ **一玩家一船,且被硬断言** | `_remoteCrafts.ContainsKey(peer.PlayerId)` 在每个入口直接 return(`:1199`、`:1515`);注册处 `:1222`、移除处 `:1290` 都按 playerId |
| **状态包携带船标识** | 包内区分"哪条船" | ❌ **无字段** | `Mod.RemoteDataPack`(`Mod.cs:162-258`)没有 craft id / 船数量 / 船数组 |
| **生命周期对账(MC3)** | 注册表 + 对账/修正 | ❌ 无 | 无注册表、无对账、无分歧检测;仅有 `CraftNodeAdded/Removed` 场景事件用于刷新本地船 |
| **`nodeId` 的用途** | 作为跨机身份基础 | ⚠️ **上线了但没用起来** | `nodeId` 确实在协议里(`MpMessage.cs:220-228`,注释明确"各机之间可重复"),但 `ApplyRemoteState` **从不读它**,只用于日志(:1249)与 `CraftDataAck` 比对(:639) |
| **残骸 / 无 pod craft** | 单独处理 | ❌ **完全缺失** | `MpNetworkManager.cs` 内 `IsDebris`/`Debris`/`残骸` 只有 1 处命中,且在被取代的死代码注释里(:1840);每玩家只有一条 `SpawnCraft` 路径(:1207) |
| **切换/对接/EVA 后的多节点** | 事件驱动同步 | ❌ 无 | 本机船切换只在场景加载(`LobbyManager.cs:158-159`)或 1.5s 重发循环(`:383-389`)时被重新识别 |
...
- `FlightState.CraftNodes` 是 `IReadOnlyList<CraftNode>`,游戏本身就管理多艘飞船(残骸、对接、多节点);
- `FlightState.AddCraft(CraftNode, originalNode)`(`FlightState.cs:320`):自动分配 NodeId、触发 `CraftNodeAdded`;
- 运行时分裂由 `CraftSplitter.SplitCraftNode` 完成:新 CraftNode → `AddCraft` → 触发 `CraftNodeAdded`,即**分离/残骸天然走同一事件通道**;
- `CraftNode.DestroyCraft()`(`CraftNode.cs:578`):置 `IsDestroyed` → 游戏 `ProcessDestroyedCraftNodes()` 下一帧移除并触发 `CraftNodeRemoved`;
...
> LunaMultiplayer 事实就是这个模型。其 `VesselLoader.LoadVessel` 还做了"结构没变就 early-out、变了才毁掉重建"的增量加载——JNO 的幽灵船模式更省(不用重建,物理本来就不开)。
...
Luna 不自己实现切换动作,而是挂钩 KSP `onVesselChange`(玩家按 `[`/`]` 或地图切换活动船时触发),在 `VesselLockEvents.OnVesselChange` 里做权限交接:

1. 目标船控制锁本来就是我的(如重载自己)→ 直接跳过,不释放锁;
2. 否则**先释放我全部 Control Lock**(一人只能控一艘);
3. 看目标船控制锁归属:
   - **别人的船** → `StartSpectating`:`IsSpectating=true` + `InputLockManager.SetControlLock(BlockAllControls)` 锁死操作 + 拿 Spectator Lock + 释放我的 Control/Update/Kerbal/UnloadedUpdate 锁;
...
切换即时性:`PositionEvents.OnVesselSwitching`(KSP `onVesselSwitching`)→ **立即广播新船位置(带 orbit/body),不等定时发包**。

兜底:`VesselSwitcherSystem.SwitchToVessel` 协程等飞船加载(100 FixedUpdate → 1s → 强制 `Load()`)再 `ForceSetActiveVessel`;杀活动船时切到最近可用船(`VesselRemoveSystem.SwitchVesselIfKillingActiveVessel`)。
...
发起端 `VesselCoupleEvents.CoupleComplete`:
...
接收端 `VesselCoupleMessageHandler` + `VesselCouple.ProcessCouple`:
...
合并后 dominant 的新结构:`VesselProtoEvents.PartCoupled` → `SendVesselMessage(dominant)` 重发合并后的完整 proto;`LocalTopologyTracker.RecordMutation` 拉黑两船 id,防止过期旧 proto 把已删 weak 船"复活"。

**分离**:
- Decouple(分离器/分级)`VesselDecoupleEvents.DecoupleComplete`:分离完成 → 立即 `SendVesselPositionUpdate(新船,true)` + 发 `VesselDecouple(原船, partFlightId, breakForce, NewVesselId)`;接收端 `VesselDecouple.ProcessDecouple` 本地 `decouple(BreakForce)`,然后 **`partRef.vessel.id = NewVesselId` 强制覆盖本地自动分配的 id** → 全局 Guid 唯一且跨机一致,再 ForceUpdate 位置。
- Undock(对接解除):新分离段作为新船发 proto(`VesselProtoEvents.PartUndocked`)。

### 7.4 JNO 同一 craft 内切换控制单元(ActiveCommandPod)

**机制**(反编译):
- API:`CraftScript.SetActiveCommandPod(pod)` + 事件 `ActiveCommandPodChanging/Changed`;统一入口 `FlightSceneScript.ChangePlayersActiveCommandPodImmediate(pod, node, ignoreDistance)`(清零控制、重定相机、`SetActiveCommandPod`、必要时 `SetCraftNode`、`AllowPlayerControl=true`)。
- 触发点:`CommandPodScript.SetActiveCommandPod()`(部件菜单"Take control",`CommandPodScript.cs:789`)、`EvaScript.TakeControl`、地图 inspector(`SelectedModel.cs:482`)、CockpitScript。
- **关键(对同步的影响)**:`CraftScript` 每帧 `_centerOfMassTransform.rotation = ActiveCommandPod.PilotSeatOrientation.rotation`。而 mod 采样朝向就是 `CenterOfMass.rotation`(→SrfRel)。**→ 同一 craft 内切换控制单元,朝向/控制自动跟随现有 srfRel 同步,无需新增包**。`Controls`(Pitch/Yaw/Roll/Throttle)也来自 `ActiveCommandPod` → 已在状态包。
...
- **出舱 EVA**(`CrewCompartmentScript.UnloadCrewMember` → `EvaScript.TakeControl` → `UnloadFromCrewCompartment` 摧毁物理关节 → body 断开):
  - 断开 body → `CraftSplitter.ProcessDisconnectedBody` → `MoveBodyToNewCraft` / `SplitCraftNode` → **生成新的独立 CraftNode**(新 NodeId;`EvaScript.OnMovedToNewCraft` 更新 `CrewMember.NodeId = newCraft.NodeId`,`EvaScript.cs:2037`);
  - `SwitchToCommandPod` → `ChangePlayersActiveCommandPodImmediate` 接管新 EVA 节点(`EvaScript.cs:2249`)。
...
**前提**:JNO 的分离有两条路径(`CraftSplitter.ProcessDisconnectedBody` → `DetermineCraftNodeEligibility`):
...
**游戏本身对无 pod craft**:照常物理模拟、在 `FlightState.CraftNodes`、地图可见;但无控制权(`SwitchToNextCommandPod` 只遍历 `HasCommandPod`;`FlightSceneScript.cs:1581`;`IsResumable = HasCommandPod && AllowPlayerControl`)。`CraftScript` 每帧覆盖朝向用 `ActiveCommandPod ?? PrimaryCommandPod`,两者都 null 时**不覆盖**(`CraftScript.cs:2046`)→ 远端幽灵朝向不会被游戏改写。生命周期:撞击/无部件/加载失败/菜单手动删(`IsDebris = !HasCommandPod && ContractTrackingId==null`)才 `DestroyCraft`;**飞行中无自动清理**。
...
## 八、<JNO_CODE> 边界情况排查(当前 plan 未覆盖 / 需补)

> 系统性扫过 `<JNO_CODE>` 生命周期/坐标/控制/资源相关路径后的补充。按优先级分三档。

### 8.1 高优先级(直接影响多 craft 正确性,必做)

1. **SOI 切换 / 换母星(Parent 变化) —【决策:暂不做跨行星】**:
   - 事实:`TransitionShipToNewPlanetSoi` → `CraftNode.TransitionToNewSoi` 改变 `craft.Parent`;玩家船还会触发 `PlayerChangedSoi` 事件。状态包的位置/速度是 **`craft.Parent` 的 surface 坐标**,朝向也相对 parent。
...
3. **远端幽灵船被 [ ] 接管风险(会炸的坑) —【决策:用 Harmony 拦截,且拦在总入口】**:
   - 事实:`SwitchToNextCommandPod` 只查 `HasCommandPod && IsLoadedInGameView`,**不查 `AllowPlayerControl`**;而 `ChangePlayersActiveCommandPodImmediate` 末尾还会强制 `AllowPlayerControl = true`(`FlightSceneScript.cs:383`)。
   ...
   - **为什么拦总入口而非 `SwitchToNextCommandPod`**:换控制不止 [ ] 一条路——地图 inspector "Take control"、部件菜单 "Take control"、`EvaScript.TakeControl`、Vizzy `CraftService.ChangePlayersActiveCommandPodImmediate`(`CraftService.cs:825`)都汇到这一个方法;拦总入口一处全覆盖。且地图/部件菜单的按钮本身因 `AllowPlayerControl=false` 已隐藏,实际漏网口主要是 [ ] 循环和直接 API 调用。

4. **JNO 原生对接 = MergeCraftNode(dominant 保身份),比 7.6 假设更简单**:
   - 事实:`DockingPortScript.CompleteDockConnection`:**玩家船 `IsPlayer` 优先为 dominant**(line 527-533 交换)→ `CraftSplitter.MergeCraftNode(source, target)`(line 552)→ source `DestroyCraft()`(`CraftSplitter.cs:73`)→ 走 `CraftNodeRemoved`。
...
5. **燃料/资源/部件状态不同步** —**【决策 2026-08:MVP 接受不同步】**:`CraftFuelSource.TotalFuel`(`CraftFuelSource.cs:236`)、part 损伤/展开/引擎/Vizzy 状态都未同步(Luna 有 `VesselResourceSystem` + `VesselPartSync*`)。幽灵物理关 → 引擎视觉本来不跑;但燃料表/性能数据会不一致。MVP 记为已知限制(用户已确认接受);后续如需再加"每 fuel source 燃料量"字段。

6. **水(浮水)**:`CraftNode.InContactWithWater`(`CraftNode.cs:363`,由 `CraftScript.cs:2141` 设置)、`FlightData.InWater`。现有 GroundedSurface 路径假设地面;浮水需同套处理 + `InContactWithWater` 标记。

7. **玩家切 craft 节点 = SetIsPlayer 切换(确认 7.5/7.6 机制)**:`SetCraftNode`(`FlightSceneScript.cs:1530`) 对旧节点 `SetIsPlayer(false)`、新节点 `SetIsPlayer(true)`,`FlightState.PlayerNodeId` 随 `ActiveCommandPodChanged` 更新。→ mod 应监听 `FlightScene.ActiveCommandPodChanged`(回调带 craftNode),NodeId 变化即 `RefreshLocalCraft()`。

### 8.3 低优先级 / 已知限制(仅记录)

8. **回收/离场景销毁**:`CraftRecovery`(菜单/地图侧,`CraftRecovery.cs:188`)、`DestroyOnExitFlightScene`(`CraftNode.cs:254`,FlightEnd 时销毁)——场景切换已被 `LobbyManager.OnSceneLoaded` 兜住。
...
11. **地图显示**:`MapViewScript.AddCraft` `IsPlayer || IsLoadedInGameView` → 动态图标,幽灵已加载会显示;配合 8.1-3 需限制选择/接管。
