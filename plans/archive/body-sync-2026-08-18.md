...
> 状态:✅ **已归档**(原状态:已实现——BodyPoses 相对 comRot 的位置+旋转,发送端采样 + 接收端 `ApplyRemoteBodyPoses` 应用,2026-09 复核通过);与 multi-craft-sync 分离为独立 plan
> 关联:[`part-switch-sync-2026-08-18.md`](part-switch-sync-2026-08-18.md)(部件开关 + P3 控制输入,转轴的输入驱动在此);[`proposals/multi-craft-sync-2026-08-16.md`](../proposals/multi-craft-sync-2026-08-16.md)(多船生命周期,分离/对接后 body 数量变化归那边);[`latency-smoothing-2026-08-22.md`](latency-smoothing-2026-08-22.md) §9.4(body 位姿的每帧平滑、数量变化重对齐在平滑层实现)

> **实现复核(2026-09)**:
> - 发送端 `TrySampleLocalCraft` 在**同一个循环**里填 `BodyRotations`(=`Inverse(comRotUnity) * body.Transform.rotation` 的欧拉角)与 `BodyPositions`(=`comRotTransform.InverseTransformPoint(body.Transform.position)`),两者**必然同长度同索引**。
> - 接收端 `ApplyRemoteBodyPoses` 用 `Mathf.Min(bodies.Count, BodyRotations.Count)` 做**越界护栏**,位置写入前再判 `i < BodyPositions.Count`;`comRot` 为空时只写旋转不写位置。
> - **每帧平滑**由平滑层负责:`alphaBody = Clamp01(10 * dt)`、位置 `sqrMagnitude < 0.01f`(0.1m)快照、旋转 `Angle < 0.01f` 度快照;body 数量变化触发 `SnapSmoothedBodies` 重建数组并逐索引兜底。
> - ⚠️ **前提约束**:幽灵物理被强制关闭,且接收端**刻意不恢复 part 位姿**(坐标口径不同会错位),body 位姿是唯一的"部件级摆放"来源——所以本 plan 的索引契约一旦破损,表现是整船错位而非轻微抖动。
> - ⚠️ 残骸/分离出的子装配**不会**变成独立幽灵(多 craft 未实现),它们只体现为同一艘幽灵内部 body 位姿的变化。
...
- **multi-craft 同步** = **多艘船之间**的身份/生命周期/对接/分离/残骸/切换。见 [`proposals/multi-craft-sync-2026-08-16.md`](../proposals/multi-craft-sync-2026-08-16.md)。
...
**1.1 根因(反编译证实)**
- `JointRotatorScript` 复用一个**既有 ConfigurableJoint**(`SetupJoint`):`_joint = bodyJoint.GetJointForAttachPoint(attachPoint)`,`_connectedRigidBody = _joint.connectedBody`(**独立 body**);
- `FlightFixedUpdate` 把 `Data.Angle` 移向 `ComputeTargetAngle()`(读输入,即 P3 已同步的 Controls)并设 `_joint.targetRotation = Quaternion.Euler(-Angle, 0, 0)`(`:86-117`)→ **物理驱动被连 body 绕铰链旋转**,其 `Transform` 跟随移动(Rigidbody 带 transform)→ 被连部件整体摆动;
- **幽灵**:所有 body `isKinematic=true` + 物理禁用 → ConfigurableJoint targetRotation **无物理效果**,被连 body 停 XML 位;但 `JointRotatorScript._visualMesh.localRotation = Euler(0,-Angle,0)`(纯 Transform,`:149`)仍跑,且 P3 后 `Data.Angle` 已按同步输入本地演进 → **铰链视觉转、被连 body 不跟**。
...
**1.3 层级与不冲突前提(已证实)**
- `body.Transform.SetParent(craftScript.Transform)`(BodyScript.cs:531);`CenterOfMass`(comRot)是根的子节点(`localPosition = localCenterOfMass`,CraftNode.cs:878);mod 每帧把根与 comRot 都转成 headingFrame → 现有 `localRotation` 写法 = 相对 comRot 旋转;
...
> 反编译源:`<SP2_MP>/Multiplayer/`。结论:**SP2 恰好验证了本方案方向正确**,且提供几个可抄的优化。

**SP2 的做法**
1. **Body 级位姿同步是"转轴/关节连接部件整体移动"的同步层次**——SP2 **不**按转轴部件同步,而是同步每个 body 相对**父 body** 的位姿:
   - `BodySyncData.Update`:`Rotation = Inverse(ParentBody.rotation)*body.rotation`;`Position = ParentBody.InverseTransformPoint(body.position)*ParentBody.lossyScale.x`;根 body(ParentBody==null)用绝对位姿;附 `AngularVelocity/Velocity`;
   - **父层级**由 SP2 独有的 `BodyConfigurationState.SetParentBody` 维护(`ParentBody.Id` 序列化);
2. **Delta 兴趣优先级 + top-N 子集**(`CraftStateSerializer.SerializeWrite`):根 body 与子 body 各自 `Delta > 0.1f` 才入列,按 Delta 降序**每包只发 top-5**;接收端按 **`body.Id`** 查找(`:115`,`AircraftScript.GetBody(id)`),**不是顺序索引**;
3. **接收端应用**(`BodyScript.cs:660-679`):子 body 写 `transform.localPosition/localRotation`(相对父 body),带 **Lerp/Slerp 平滑**(`10f*Time.deltaTime`,偏差 <0.01 直接快照);根 body 写 RigidBody + velocity(SP2 远程船**物理保持开启**,靠 lerp 收敛);
4. **per-part 连续状态**:`PartSyncData` + `ISyncValue`(`SyncFloat/SyncVector/SyncBool`,`SyncValue.cs:31-36` 的 `Value`(采样 Func)/`ValueRead`(应用 Action)/`LastValueSent`+`Delta`),按注册序序列化、每包 top-5 by delta;用于**非 body 连续状态**:轮子 RPM(`ResizableWheelScript.cs:231`)、翼涡流数据、炮口闪光(`CannonScript.cs:451`)、发动机 throttle、螺旋桨桨距(`PropellerAssemblyScript.cs:435`);
...
- **2026-08-18**:方案定稿(BodyPoses,SP2 验证);与 multi-craft-sync 分离为独立 plan;实现 P0(BodyPositions 平行列表,采样+两处应用)。
  ...
  - 编译:`dotnet build MultiPlayer.csproj -c Debug` **0 错误 0 警告**。
